...
```{r}
df %>% select(time, sim, plus_state, true_state) %>% gather(model, state, -time, -sim) %>% 
ggplot(aes(x=time, y=state, color = model, group = interaction(model,sim))) + geom_line()
```



```{r}
df %>% select(time, sim, plus_reward, true_reward) %>% gather(model, reward, -time, -sim) %>% 
ggplot(aes(x=time, y=reward, color = model, group = interaction(model,sim))) + geom_line()
```



```{r}
df %>% select(time, sim, plus_action, true_action) %>% gather(model, action, -time, -sim) %>% 
ggplot(aes(x=time, y=action, color = model, group = interaction(model,sim))) + geom_line()
```
