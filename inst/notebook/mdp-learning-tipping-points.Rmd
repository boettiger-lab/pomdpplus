...
value_of_information <- function(x){
  ...
  transition <- x[[2]]
  model <- x[[3]]
  low_low =   value_of_policy(policies$low, transition[[2]],   utility, discount)
  high_high = value_of_policy(policies$high, transition[[11]], utility, discount)
  unif_low = value_of_policy(policies$unif, transition[[2]], utility, discount) / low_low
  high_low = value_of_policy(policies$high, transition[[2]], utility, discount)  / low_low
  unif_high = value_of_policy(policies$unif, transition[[11]], utility, discount) / high_high
  low_high =  value_of_policy(policies$low, transition[[11]],  utility, discount)  / high_high
...
```{r}
sim$df %>% select(-value) %>% gather(series, stock, -time, -rep) %>%
  ggplot(aes(time, stock, color = series, group = rep)) + geom_line(alpha = 0.5)
