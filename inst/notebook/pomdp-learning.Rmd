...
```{r}
alphas <- init_models(K_models, discount, precision = 1)
```
...
compare_policies <- function(alphas, models){
  low <- compute_policy(alphas, models, c(1, 0, 0))
  unif <- compute_policy(alphas, models, c(1/3, 1/3, 1/3))
  high <- compute_policy(alphas, models, c(0, 0, 1))
