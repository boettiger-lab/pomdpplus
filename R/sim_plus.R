...
#' (3) \code{state_posterior}, an array of Tmax rows by n_states by n_models columns giving the evolution of the belief over states
#' @export
#'
#' @examples \dontrun{
...
sim_plus <- function(models, discount, model_prior = NULL, state_prior = NULL,
  ...
  state_posterior <- array(NA, dim = c(Tmax+1, n_states, n_models))
  ...
  state_posterior[2,,] <- vapply(1:n_models, function(i) state_prior, numeric(n_states))
  ...
  for(t in 2:Tmax){
    ...
    model_posterior[t+1,] <- update_model_belief(model_posterior[t,], state_posterior[t,,], models, obs[t], action[t-1])
    state_posterior[t+1,,] <- update_state_belief(model_posterior[t,], state_posterior[t,,], models, obs[t], action[t-1])
...
update_model_belief <-  function(model_prior, state_prior, models, z_t, a_t){
  belief <-
    model_prior *
    vapply(1:length(models), function(i){
      state_prior[,i] %*%
        models[[i]]$transition[, , a_t] %*%
        models[[i]]$observation[, z_t, a_t]
    }, numeric(1))
  belief / sum(belief)
}

update_state_belief <- function(model_prior, state_prior, models, z_t, a_t){
  n_states <- dim(state_prior)[[1]]
  n_models <- length(models)
  belief <-
    vapply(1:n_states, function(i){ sum(    ## belief for each state, p_{t+1}(x_{t+1})
      vapply(1:n_models, function(k){ ## average over models
        model_prior[k] *
          state_prior[,k] %*%
          models[[k]]$transition[, i, a_t] *
          models[[k]]$observation[i, z_t, a_t]
      }, numeric(1)))
    }, numeric(1))
